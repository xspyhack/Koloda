...
private let defaultBackgroundCardsScalePercent: CGFloat = 1.0
...
open class KolodaView: UIView, DraggableCardDelegate {
    ...
    open func insertCardAtIndexRange(_ indexRange: CountableClosedRange<Int>, animated: Bool = true) {
        ...
        let visibleIndexes = Array(indexRange).filter { $0 >= currentCardIndex && $0 < currentCardIndex + countOfVisibleCards }
    ...
    open func removeCardInIndexRange(_ indexRange: CountableClosedRange<Int>, animated: Bool) {
        ...
        let currentItemsCount = countOfCards
        countOfCards = Int(dataSource.kolodaNumberOfCards(self))
        let visibleIndexes = indexRange.filter { $0 >= currentCardIndex && $0 < currentCardIndex + countOfVisibleCards }
        if !visibleIndexes.isEmpty {
            proceedDeletionInRange(visibleIndexes[0]...visibleIndexes[visibleIndexes.count - 1])
        }
        currentCardIndex -= indexRange.filter { $0 < currentCardIndex }.count
    ...
    open func reloadCardsInIndexRange(_ indexRange: CountableClosedRange<Int>) {
        ...
        let visibleIndexes: [Int] = Array(indexRange).filter { $0 >= currentCardIndex && $0 < currentCardIndex + countOfVisibleCards }
